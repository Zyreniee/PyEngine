...
Renderer::Renderer(Window &window) : m_Window(window) {
  ...
  m_Swapchain = std::make_unique<Swapchain>(*m_Context, window.GetWidth(),
                                            window.GetHeight());
...
void Renderer::BeginFrame() {
  PYENGINE_CORE_ASSERT(!m_IsFrameStarted,
                       "Cannot call BeginFrame while already in progress");

  vkWaitForFences(m_Context->GetDevice(), 1,
                  &m_InFlightFences[m_CurrentFrameIndex], VK_TRUE, UINT64_MAX);
  ...
  PYENGINE_CORE_ASSERT(result == VK_SUCCESS || result == VK_SUBOPTIMAL_KHR,
  ...
  renderPassInfo.framebuffer =
      m_Swapchain->GetFramebuffers()[m_CurrentImageIndex];
  ...
  vkCmdBeginRenderPass(m_CommandBuffers[m_CurrentImageIndex], &renderPassInfo,
...
void Renderer::EndFrame() {
  PYENGINE_CORE_ASSERT(m_IsFrameStarted,
                       "Cannot call EndFrame if frame is not in progress");
  ...
  VkResult result = vkQueueSubmit(m_Context->GetGraphicsQueue(), 1, &submitInfo,
                                  m_InFlightFences[m_CurrentFrameIndex]);
...
void Renderer::DrawMesh(Mesh *mesh, const glm::mat4 &transform) {
  ...
  vkCmdBindDescriptorSets(m_CommandBuffers[m_CurrentImageIndex],
                          VK_PIPELINE_BIND_POINT_GRAPHICS, m_PipelineLayout, 0,
                          1, &m_DescriptorSets[m_CurrentImageIndex], 0,
                          nullptr);
...
void Renderer::CreateRenderPass() {
  ...
  VkResult result = vkCreateRenderPass(m_Context->GetDevice(), &renderPassInfo,
...
void Renderer::CreatePipeline() {
  ...
  m_Pipeline = std::make_unique<Pipeline>(*m_Context, vertPath, fragPath,
                                          m_PipelineLayout, config);
}
...
void Renderer::CreateCommandPool() {
  ...
  VkResult result = vkCreateCommandPool(m_Context->GetDevice(), &poolInfo,
...
void Renderer::CreateCommandBuffers() {
  ...
  VkResult result = vkAllocateCommandBuffers(m_Context->GetDevice(), &allocInfo,
...
void Renderer::CreateUniformBuffers() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    vmaCreateBuffer(m_Allocator, &bufferInfo, &allocInfo, &m_UniformBuffers[i],
                    &m_UniformBufferAllocations[i], &allocationInfo);
...
void Renderer::CreateDescriptorPool() {
  ...
  VkResult result = vkCreateDescriptorPool(m_Context->GetDevice(), &poolInfo,
...
void Renderer::CreateDescriptorSets() {
  ...
  VkResult result = vkAllocateDescriptorSets(m_Context->GetDevice(), &allocInfo,
