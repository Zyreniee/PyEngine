...
void Swapchain::CreateSwapchain() {
  ...
  VkResult result = vkCreateSwapchainKHR(m_Context.GetDevice(), &createInfo,
                                         nullptr, &m_Swapchain);
  ...
  PYENGINE_CORE_INFO("Swapchain created: {} images, {}x{}", imageCount,
...
void Swapchain::CreateImageViews() {
  ...
  for (size_t i = 0; i < m_Images.size(); i++) {
    ...
    VkResult result = vkCreateImageView(m_Context.GetDevice(), &createInfo,
...
void Swapchain::CreateDepthResources() {
  ...
  VkResult result =
      vkCreateImage(m_Context.GetDevice(), &imageInfo, nullptr, &m_DepthImage);
  ...
  PYENGINE_CORE_ASSERT(memoryTypeIndex != UINT32_MAX,
                       "Failed to find suitable memory type!");
  allocInfo.memoryTypeIndex = memoryTypeIndex;

  result = vkAllocateMemory(m_Context.GetDevice(), &allocInfo, nullptr,
                            &m_DepthImageMemory);
  ...
  result = vkCreateImageView(m_Context.GetDevice(), &viewInfo, nullptr,
                             &m_DepthImageView);
  PYENGINE_CORE_ASSERT(result == VK_SUCCESS,
...
void Swapchain::CreateFramebuffers(VkRenderPass renderPass) {
  ...
  for (size_t i = 0; i < m_ImageViews.size(); i++) {
    ...
    VkResult result = vkCreateFramebuffer(
        m_Context.GetDevice(), &framebufferInfo, nullptr, &m_Framebuffers[i]);
