...
int main() {
    ...
    // Create ImGui layer
    auto imguiLayer = std::make_unique<PyEngine::ImGuiLayer>(window, renderer->GetContext(), *renderer);
    ...
    // Create cube mesh
    auto cubeMesh = PyEngine::Mesh::CreateCube(renderer->GetContext(), renderer->GetAllocator());
    ...
    while (!window.ShouldClose()) {
        ...
        if (renderer->IsFrameInProgress()) {
            ...
            // Draw rotating cube
            glm::mat4 transform = glm::rotate(glm::mat4(1.0f), glm::radians(rotation), glm::vec3(0.5f, 1.0f, 0.2f));
            ...
            ImGui::Text("Camera Forward: %.2f, %.2f, %.2f", camera.GetForward().x, camera.GetForward().y,
                        camera.GetForward().z);
            ImGui::Separator();
            ImGui::Text("Controls:");
